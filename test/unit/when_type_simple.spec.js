...
test('when type simple - valid schema', () => {
  expect(jsonSchema).toEqual(BASE_SCHEMA)
})

test('when type simple - valid matching if', () => {
  const value = 50
  validate(value)
...
test('when type simple - valid matching otherwise', () => {
  const value = 8
  validate(value)
  expect(validate.errors).toBeFalsy()
  expect(JOI_OBJ.validate(value).error).toBeFalsy()
...
test('when type simple - invalid matching if', () => {
  const value = 110
  validate(value)
...
test('when type simple - invalid matching otherwise', () => {
  const value = 19
  validate(value)
  expect(validate.errors).toBeTruthy()
  expect(JOI_OBJ.validate(value).error).toBeTruthy()
